...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SE FUE/FUE EXPULSADO POR GAI O LESBIANA🥳 @${num.split('@')[0]} :V²`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '❬❗❭  SOLO CREADOR ',
					admin: '❬❗❭ SOLO DIOSES ',
					Badmin: '❬❗❭ BOT NECESITA SER DIOS :V '
				}
			}
          ...
          case 'ping':
            ...
            break	
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'nombre de usurio cual es?', text, {quoted: mek})
						...
						reply('[𝗘𝗥𝗥𝗢𝗥]Nombre de usuario incorrecto')
					}
					break
				case 'fitnah':	
				case 'fake':          
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `YA NO ERES DIOS🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} YA NO ERES DIOS🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`FELICIDADES🥳 @${mentioned[0].split('@')[0]} ERES DIOS (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `FUE EXPULSADO POR GAI/LESGIANA 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`FUE EXPULSADO POR GAI/LESBIANA @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ EXITO ❭ BIENVENID@/GOODBYE AL GRUPO PUT@')
					} else if (Number(args[0]) === 0) {
